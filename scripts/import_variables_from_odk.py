...
### variedad ###
survey = pd.read_excel(open(sys.argv[1], 'rb'), sheet_name='survey')
survey.rename(columns={"label": "label_spanish", "name": "value"}, inplace=True)
survey = survey.where((pd.notnull(survey)), None)
survey = survey.dropna(thresh=1)
